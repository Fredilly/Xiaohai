...
try {
  ...
  console.log(`HQ: ${result.hqLoginIdentifier} · GLOBAL · ${result.hqPermissionCount} permissions`);
  console.log(
    `Store: ${result.storeLoginIdentifier} · ${result.storeName} (${result.storeId}) · ${result.storePermissionCount} permissions`,
  );
  console.log('Passwords were read from environment variables and were not printed or persisted in source.');
} catch (error) {
