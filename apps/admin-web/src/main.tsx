  ...
  return (
    <main className="login-page">
      <section className="login-card">
        ...
        <h1>总部运营后台</h1>
        <p className="muted">
          统一管理内容、门店与业务运营。真实权限始终由服务端 RBAC + Data Scope 决定。
        </p>
  ...
  return (
    <>
      <section className="hero">
        <div>
          <span className="badge">前端 Sprint</span>
          <h2>运营框架已就位，业务能力按里程碑接入</h2>
          <p>这里的业务数字均为前端演示状态，不代表真实订单、库存或财务数据。</p>
        </div>
      ...
      <section className="stats">
        <article>
          <span>今日待办</span>
          <strong>—</strong>
          <small>等待正式业务 API</small>
        </article>
        <article>
          <span>门店网络</span>
          <strong>Preview</strong>
          <small>组织 API 尚未实现</small>
        </article>
        <article>
          <span>订单中心</span>
          <strong>Preview</strong>
          <small>订单 API 尚未实现</small>
        </article>
      </section>
...
function Preview({
  ...
  return (
    <section className="panel preview">
      <span className="badge">Frontend preview · 非真实业务数据</span>
      <h2>{title}</h2>
      <p>{description}</p>
      <div className="empty-state">
        <strong>页面框架已准备</strong>
        <span>
          正式
          API、数据库业务模型与写操作将在对应里程碑实现。当前页面不会模拟支付成功、库存扣减或真实财务入账。
        </span>
      </div>
      {title === 'Staff / 权限' && (
        <div className="context">
          <h3>当前服务端授权上下文</h3>
          <p>Permissions: {me.permissions.length ? me.permissions.join(' · ') : '暂无'}</p>
          <p>
            Data Scopes:{' '}
          ...
          <small>这些值来自 GET /api/v1/staff/me，仅用于展示；前端可见性不是授权边界。</small>
        </div>
      )}
    </section>
  );
