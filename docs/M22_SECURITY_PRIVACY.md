...
Pino 以白名单序列化请求 ID 与方法，不记录请求 URL、header、body 或 query；密码、会话、微信身份、密钥和异常对象使用集中脱敏。审计关联的 request ID 由服务端生成，不接受客户端自选值。公共未处理错误返回统一代码和 request ID，不返回原始异常。密钥扫描拒绝本地环境文件、私钥、证书及常见服务 token；示例值允许保留。
Pino serializes only request ID and method, omitting URL, headers, body and query; passwords, sessions, WeChat identifiers, keys and exception objects receive centralized redaction. Request IDs used for audit correlation are generated by the server, not chosen by clients. Unhandled public errors return a stable code and request ID, not raw exceptions. Secret scanning rejects local environment files, private keys, certificates and common service tokens while allowing example values.
