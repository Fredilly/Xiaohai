...
`API_REQUEST_METRIC` logs route templates, status and duration without raw URL, query or headers. `/health` is liveness; `/ready` checks PostgreSQL and Redis and returns only ready/unavailable. Schedule `packages/db/scripts-reliability-scan.mjs` under a read-only DB role. Its shared SQL runs against migrated PostgreSQL in the dedicated CI integration test, which checks an empty baseline, a historical `FAILED` job, a stuck job, a finance reconciliation discrepancy and rollback without persistent writes.

The scanner separates **invariant alerts** (payment/refund amounts, payment and commission finance projections, inventory and latest reconciliation discrepancies) from **stuck-work alerts** (RUNNING AI/image/video/composition beyond the documented detection window). Either category returns exit 1 when positive. The payment reconciliation check uses the latest item per payment; the finance check uses the latest completed finance run, so resolved historical discrepancies do not alert forever. `ai_failed_total` counts historical terminal failures as a **metric only**: its positive count does not fail the scan. A failure-spike alert threshold requires staging baseline and an operations decision; do not alert merely because a job has ever failed. Wire structured events to the chosen monitoring system at deployment; keep labels bounded and strip PII.
...
Formal latency/availability/RPO/RTO targets; realistic staging data volume and traffic profile; historical failure-spike threshold based on staging baseline; alert owner/escalation and monitoring vendor; production backup encryption, retention and restore schedule; CDN provider, private media access and cache policy; provider idempotency contract for image/video/composition generation. Production restore and cutover require a separate authorized operation. M23 is not production Done until realistic staging load, production CDN behavior and monitoring integration are validated.
