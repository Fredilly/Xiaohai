...
export const seriesDetailSchema = seriesSummarySchema.extend({
  ...
  recommendations: z.array(seriesSummarySchema),
});
export const contentListResponseSchema = z.object({ series: z.array(seriesSummarySchema) });
export const playbackAccessSchema = z.object({
  episodeId: z.uuid(),
  access: z.enum(['FULL', 'PREVIEW', 'LOCKED']),
  playbackUrl: z.string().url().nullable(),
  previewSeconds: z.number().int().positive().nullable(),
  resumePositionSeconds: z.number().int().nonnegative(),
});
export const progressUpdateSchema = z
  .object({ positionSeconds: z.number().int().nonnegative(), completed: z.boolean() })
  .strict();
export const progressSchema = progressUpdateSchema.extend({
  episodeId: z.uuid(),
  updatedAt: z.string(),
});
export const continueWatchingSchema = z.object({
  items: z.array(
    z.object({ series: seriesSummarySchema, episode: episodeSchema, progress: progressSchema }),
  ),
});
export const entitlementListSchema = z.object({
  series: z.array(seriesSummarySchema.extend({ grantedAt: z.string() })),
});
...
export const episodeInputSchema = z
  ...
  .strict()
  .superRefine((v, ctx) => {
    if (v.accessMode === 'PREVIEW' && !v.previewSeconds)
      ctx.addIssue({
        code: 'custom',
        message: 'previewSeconds required for PREVIEW',
        path: ['previewSeconds'],
      });
  });
